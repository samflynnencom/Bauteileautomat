...
#define ELPOS_STEP_MASK			((uint8_t)0xC0)
#define ELPOS_MICROSTEP_MASK		((uint8_t)0x3F)

/* dSPIN min speed register bit / mask */
#define LSPD_OPT			((uint16_t)0x1000)
#define MIN_SPEED_MASK	((uint16_t)0x0FFF)
...
void dSPIN_SetParam(int device, uint8_t param, uint32_t value); 	//done
unsigned long dSPIN_GetParam(int device, uint8_t param);					//done
...
uint8_t SoftSSI_DataGet(uint32_t SSIPortBase);
uint8_t	SoftSSI_Busy(uint32_t SSIPortBase);
uint8_t SoftSSI_Enable(uint32_t SSIPortBase);


uint8_t dSPIN_Xfer(int device, uint8_t data);			//done
uint32_t dSPIN_Write(int device, uint32_t data, uint8_t bitLen);//done
unsigned long dSPIN_ParamHandler(int device, uint8_t param, unsigned long value);//done
